...
public class AgileEnemy : Enemy
{
    ...
    protected override void UpdateEnemyState()
    {
        ...
        if (currentState == EnemyState.Walk)
        {
            ...
            else if (playerDetected && !wasPlayerDetectedLastFrame && Time.time >= lastDashTime + config.dashCooldown && !isDashing)
            {
        ...
        else if (currentState == EnemyState.QuickAttack)
        {
            enemyRb.linearVelocity = Vector2.zero;
        }
    ...
    private IEnumerator DashCoroutine(float moveDirection)
    {
        isDashing = true;
        lastDashTime = Time.time;
        enemyRb.linearVelocity = new Vector2(moveDirection * config.speed * config.dashSpeedMultiplier, enemyRb.linearVelocity.y);
    ...
    public void OnAttackHit()
    {
        ...
        if (IsPlayerInAttackZone() && pc != null && !pc.IsInvincible)
        {
            Vector2 hitDirection = (pc.transform.position - transform.position).normalized;
            pc.TakeDamage(config.quickDamage, hitDirection, 2f);
            AudioManager.Instance?.PlaySound(hitSound);
            Debug.Log($"AgileEnemy dealt {config.quickDamage} damage to player.");
        }
